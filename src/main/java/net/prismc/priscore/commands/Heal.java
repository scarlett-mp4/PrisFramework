...
public class Heal extends UtilApi implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (p.hasPermission("prismc.heal")) {
                if (args.length == 0) {
                    p.setFoodLevel(20);
                    p.getPlayer().setHealth(20.0D);
                    p.sendMessage(getString(lang, "Commands.Heal.Healed"));

                } else if (args.length == 1) {
                    try {
                        PrisBukkitPlayer t = PrisCoreApi.wrapPrisBukkitPlayer(Bukkit.getPlayer(args[0]));
                        t.setFoodLevel(20);
                        t.getPlayer().setHealth(20.0D);
                        t.sendMessage(getString(t.getLanguageFile(), "Commands.Heal.Healed"));
                        String message = PLAYER_PATTERN.matcher(getString(lang, "Commands.Heal.HealedOthers")).replaceAll(t.getName());
            ...
            if (args.length == 1) {
                try {
                    ...
                    p.setFoodLevel(20);
                    p.getPlayer().setHealth(20.0D);
                    p.sendMessage(getString(p.getLanguageFile(), "Commands.Heal.Healed"));
                    Bukkit.getServer().getConsoleSender().sendMessage(ChatColor.LIGHT_PURPLE + "You have healed " + args[0] + "!");
