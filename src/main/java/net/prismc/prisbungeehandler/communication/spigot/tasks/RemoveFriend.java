...
public class RemoveFriend extends SpigotTask {
    ...
    public void executeTask(PrisPlayer p, Server server, JsonObject object) {
        ProxyServer.getInstance().getPluginManager().dispatchCommand(ProxyServer.getInstance().getPlayer(p.getUniqueId()), "friend remove " + object.get("friendname").getAsString());
    }
}
